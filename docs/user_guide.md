...
本系統是由 FastAPI 後端、APScheduler 排程引擎、雙 SQLite 資料庫與前端 Glassmorphism 暗黑霓虹風網頁面板所組成的「一體化台股量化工作站」。

```mermaid
graph TD
    A[瀏覽器 Frontend Dashboard] -->|CRUD/Sync/Backtest| B[FastAPI Web Server]
    A -->|切換資料來源| G[config.json]
    B -->|讀取/切換來源| G
    B -->|定時排程週一至週五 13:40| C[APScheduler 引擎]
    B -->|Shioaji 模式| D1[(Shioaji.db)]
    B -->|Yahoo 模式| D2[(Y.db)]
    B -->|API 請求| E[Shioaji API 券商接口]
    B -->|網路爬蟲 / API| H[Yahoo Finance API]
    E -->|1K 原始資料下載| B
    H -->|歷史 K 線下載| B
    B -->|內存 Pandas Resample| D1
    B -->|內存 Pandas Resample| D2
    B -->|執行量化回測| F[Backtester 引擎]
    F -->|輸出報表與日誌| A
```

本系統的四大支柱為：
1. **雙資料來源與資料庫隔離**：支援永豐金 Shioaji API 與 Yahoo Finance 雙數據來源，分別寫入獨立的 `Shioaji.db` 與 `Y.db` 資料庫，確保數據互不污染。
2. **願望清單管理與同步排程**：定時下載、智慧斷點續傳。
3. **多週期四宮格看盤面板**：5M/15M/60M/1D 量價共振，支援 SMC 區間動態開關。
4. **多策略量化回測引擎**：支援雙向交易、波段持倉、並能精確複盤。
...
### 6. 雙資料來源切換與獨立資料庫機制
* **資料來源切換**：在 Dashboard 網頁右上角設有 Glassmorphic 霓虹風的 **「資料來源切換開關 (Shioaji / Yahoo)」**。切換後，系統會即時更新 [config.json](file:///c:/Intel/Shioaji_stock/config.json) 中的設定，並動態將資料庫連線路由切換至對應的 SQLite 檔案。
* **資料庫完全隔離**：
  * **Shioaji 模式**：讀寫 [Shioaji.db](file:///c:/Intel/Shioaji_stock/Shioaji.db) 資料庫。
  * **Yahoo 模式**：讀寫 [Y.db](file:///c:/Intel/Shioaji_stock/Y.db) 資料庫。
  * 兩個資料庫獨立運作，結構完全一致，但資料來源與下載規則各自獨立，確保回測與看盤數據互不污染。
* **Yahoo Finance 同步特色與限制**：
  * **環境變數自定義下載天數**：您可以在 [.env](file:///c:/Intel/Shioaji_stock/.env) 中調整 Yahoo 模式的歷史資料回溯天數：
    * `YAHOO_60K_BACKTRACK_DAYS=365`：預設下載最近 1 年的 60m 數據。
    * `YAHOO_1D_BACKTRACK_DAYS=1825`：預設下載最近 5 年的日K (1d) 數據。
  * **短週期 (5m/15m/30m) 60 天下載限制**：由於 Yahoo Finance API 限制 5 分鐘、15 分鐘與 30 分鐘等短週期數據只能取得最近 60 天內的資料（否則會回傳錯誤），系統在 Yahoo 同步時短週期會自動限制最大下載天數為 **55 天** (`min(DEFAULT_START_DAYS, 55)`)，以確保同步任務穩定不中斷。
  * **時區自動對齊**：Yahoo Finance 抓取的歷史數據時間戳為 UTC，系統下載後會**自動轉換並對齊台北標準時區 (UTC+8)**，確保與台股交易時間軸（09:00 - 13:30）完美對齊。
  * **防封鎖安全延遲**：Yahoo 同步時，在個股之間加入了 **1.0 秒的冷卻延遲**，以防高頻請求被 Yahoo 伺服器拒絕。
...
* **資料來源感知**：看盤頁面右上角亦會顯示目前所屬的資料來源狀態（Shioaji 或 Yahoo），並從當前啟用的資料庫（[Shioaji.db](file:///c:/Intel/Shioaji_stock/Shioaji.db) 或 [Y.db](file:///c:/Intel/Shioaji_stock/Y.db)）中讀取對應週期的 K 線數據。
...
* **回測分K週期**：可選擇以 `5k`、`15k` 或 `60k` 作為回測交易週期（LTF）。
* **策略選擇**：
...
### 4. Yahoo 模式回測隔離防護機制
* **自動補齊限制**：在 **Shioaji 模式**下執行回測時，若資料庫中回測時間區間的歷史資料不足，系統會自動呼叫券商 API 下載 1K 原始資料並補齊。但在 **Yahoo 模式**下，為避免自動補齊機制寫入 [Y.db](file:///c:/Intel/Shioaji_stock/Y.db) 造成資料庫污染（或因 Shioaji API 憑證未連線導致異常），系統已實施**隔離防護**。
* **資料不足提示**：若在 Yahoo 模式下回測且資料不足，系統**不會**自動下載補齊，而是會直接跳出明確提示，引導使用者回到主頁，先手動點擊「同步數據」補齊該股票在 Yahoo 模式下的歷史資料，然後再進行回測。
